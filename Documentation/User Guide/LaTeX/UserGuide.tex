...
but the approach becomes more complex---but
...
First, a sizeable investment may be required
...
as the value of
\envvar{CMAKE_C_COMPILER}
and the location of the {\CC}
compiler as the value
of \envvar{CMAKE_CXX_COMPILER}.
...
It is possible to build {\CppTransport} executables by hand, but this
...
{\CMake} tools which are intended to make simplify this process.

The {\CMake} build script should be called \file{CMakeLists.txt} and
placed it the same directory as the
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \end{minted}
        This advises {\CMake} that the block gives a recipe for constructing
        the files \file{dquad\_core.h} and \file{dquad\_mpi.h},
        in a way which depends on the file \file{dquad.model}.
        The {\CppTransport} translator is invoked with the options
        ...
        extra compiler flags which are specific to the
        \cmakevar{dquad} target.
        The flag \cmakevar{-std=c++14} is required.
        ...
        On AMD they are currently implemented only for the Carrizo platform.
        ...
        because not all machines may have the same instruction set enhancements
        available.
        If this happens, the executable will terminate with an error message.
    \end{enumerate}
\end{enumerate}
The same pattern can be adapted for any {\CppTransport} executable.
