...
\begin{enumerate}
    ...
    \begin{tabular}{p{2.5cm}p{11.2cm}}
        ...
        The quantity stored is $k^3 \Sigma^{ab}$, where $k = |\vect{k}_1| = |\vect{k}_2|$
    ...
    \begin{tabular}{p{2.5cm}p{11.2cm}}
        ...
        of the tensor two-point function
...
\begin{itemize}
    ...
    functions, and therefore while the choice of $\astar$ has no effect
    of the final values, it does affect the magnitude of intermediate
    terms.
    This may change in future, although if so this is likely to happen in a way
    ...
    to improve its performance by adjustment of $\astar$.
...
phase.
The content group generated by such integration tasks
...
function.
If we wish to implement pairing then
...
For this reason there is no real use case for unequal checkpoint intervals.

\subsection{Using $\zeta$ {\SQLite} data containers}

\subsection{Inner product tasks to compute $\fNL$-like amplitudes}

\para{Using the {\SQLite} data container}
