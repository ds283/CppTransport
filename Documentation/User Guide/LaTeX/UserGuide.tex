...
including
\href{https://www.open-mpi.org}{\packagefont OpenMPI},
\href{https://www.mpich.org}{\packagefont MPICH}
(or its derivatives)
or the
\href{https://software.intel.com/en-us/intel-mpi-library}{\packagefont Intel MPI} libraries.
...
\begin{enumerate}
    ...
    {\MacPorts} to install the Boost libraries using {\packagefont OpenMPI}
    ...
    \end{minted}
    This selects {\packagefont OpenMPI} as the default {\MPI}
    implementation,
...
\begin{itemize}
    ...
    \item \option{{-}{-}no-colour} \\
    Do not produce colourized output. {\CppTransport} will normally detect
    the type of terminal under which it is running, and will adjust its
...
To make the executable \file{dquad} useful we must add an integration task.
This is done by using the task manager's
\mintinline{c++}{add_generator()}
method to inform it that this executable is capable of
generating tasks.
This method takes one argument, which should be a callable
object which accepts a reference to
...
these databases and provides their services to other
...
then the simplest option is to construct a class
which provides a call operator.
For the double-quadratic model this would take the form
\begin{minted}{c++}
	class write_task
	  {
	  public:
	    write_task(transport::dquad_mpi<>* m) : model(m) {}
	
	  public:
	    void operator()(transport::repository<>& repo);
	
	  private:
	    transport::dquad_mpi<>* model;
	  };	
\end{minted}
This declaration should be inserted above
\mintinline{c++}{main()}.
The \mintinline{c++}{write_task} constructor
expects to be passed a pointer to a model instance, which
will be the model used to construct any tasks which it generates.

With this declaration in place we should now call
\mintinline{c++}{add_generator()} with a
\mintinline{c++}{write_task} instance.
This is done by inserting the lines
\begin{minted}{c++}
    // register task writer
    mgr.add_generator(write_task(model.get()));
\end{minted}
immediately prior to the call to
\mintinline{c++}{mgr.process()}.

\para{Building a task}
The final step is to use the \mintinline{c++}{operator()} method of
\mintinline{c++}{write_task} to construct an integration task
and write it into the repository.
...
This includes:
\begin{itemize}
	\item details of the model itself, identified
	through the pointer to the model instance
	passed to the \mintinline{c++}{write_task} constructor
	
	\item specific choices for any parameters
	used in the Lagrangian,
	and a value for the Planck mass $\Mp$
	
	\item specific choices for the initial values of the background
...
These options are also recorded in the integration task.
...
\end{minted}
To aid readability it is a good idea to use named temporary variables
which give meaning to numbers which are quoted directly.
...
\begin{itemize}
	...
	for their derivatives $\d \phi^\alpha / \d N$.
...
quoting it directly using an initialization list.
If a number of values other than $N$ or $2N$ is given then
{\CppTransport} will raise
a \mintinline{c++}{std::out_of_range} exception.

In addition, the package
requires information about where these
initial conditions are intended to apply.
...
\begin{itemize}
	...
	together with the horizon-crossing
	time for $\kstar$ (at $N=\Nstar$),
	...
	If the slow-roll approximation is a reasonable approximation
	...
	Provided $\Ninit$ is sufficiently early, the customization
	procedure will allow the initial conditions to relax onto
...
to construct arbitrary ranges with either
linear or logarithmic spacing
The building block
is a class called
\mintinline{c++}{transport::basic_range<>}. This has the basic syntax
\begin{center}
...
\end{itemize}
For any spacing type, if $N=0$ the range consists of a single
value equal to \mintinline{c++}{lo}.

Any number of \mintinline{c++}{basic_range<>} ranges can be
added to produce a composite range.
...
which allows the compiler to deduce the appropriate type.
...
For time sample, a sensible starting point is to sample linearly
in $N$
...
For the two-point function, a wavenumber configuration is fixed simply
by the magnitude $k$.
A set of samples can therefore be specified by a range
(possibly a composite, as above).
The wavenumber $k=1$ is defined to exit the horizon at the time
$N = \Nstar$
fixed by the initial conditions package.
If $H$ is nearly constant then a general wavenumber $k$ will exit the
horizon approximately at time $N = \Nstar + \ln k$.
This is usually a good rule-of-thumb when deciding what range $k$ cover
a given range of e-folds.
...
To begin, we will construct a range of wavenumbers which
...
integration tasks.
{\CppTransport} offers two options.
Although it will compute the 3-point function, doing so is expensive
and wasteful if it is not required.
Therefore the first option is to compute the 2-point function
alone.
This is represented by an object of type
\mintinline{c++}{transport::twopf_task<>}:
\begin{minted}{c++}
    transport::twopf_task<> tk2("dquad.twopf", ics, ts, ks);
\end{minted}
The constructor requires a name, an initial conditions package,
...
there are many other options, which will be described fully in the next
section.

\subsection{Committing tasks to the database}
