...
\begin{itemize}
    ...
    Alternatively, it is possible to abandon the use of an SQL database entirely
    ...
    more difficult to use---in particular,
    data analysis would need to be performed using
    purpose-written code rather than SQL queries and CSV files,
    because HDF5 is mostly searchable only via an API.
    ...
    Otherwise, at present, contention from multiple workers often sets a
...
tasks will be quite sufficient to carry out a
...
This data has to be retrieved from the main {\SQLite} container
\file{data.sqlite}
