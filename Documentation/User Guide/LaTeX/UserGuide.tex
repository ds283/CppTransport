...
\BeforeBeginEnvironment{minted}{\begin{tcolorbox}[enhanced,breakable,colback=white,boxrule=0pt,frame hidden,top=-0.75mm,bottom=-0.75mm,left=-0.75mm,right=-0.75mm]}
\AfterEndEnvironment{minted}{\end{tcolorbox}\noindent}

\newenvironment{example}{\begin{tcolorbox}[enhanced,colback=white,colbacktitle=black!20,colframe=black!40,coltitle=black,title=Example,fonttitle=\sffamily\fontseries{b}\selectfont]}{\end{tcolorbox}}
...
\definecolor{ERCorange}{HTML}{F26623}
\definecolor{SussexCobaltBlue}{HTML}{1E428A}
\definecolor{SussexDeepAquamarine}{HTML}{487A7B}
\hypersetup{colorlinks=true,citecolor=SussexDeepAquamarine,linkcolor=SussexCobaltBlue,urlcolor=SussexCobaltBlue}
...
\newcommand{\newfeature}[1]{\textcolor{SussexCobaltBlue}{({\packagefont New in {#1}})}}
...
\newcommand{\PyTransport}{{\packagefont PyTransport}}
...
\newcommand{\SPLINTER}{{\packagefont SPLINTER}}
\newcommand{\JsonCpp}{{\packagefont JsonCPP}}
\newcommand{\Eigen}{{\packagefont Eigen}}

\newcommand{\Boost}{{\packagefont Boost}}
\newcommand{\odeint}{{\packagefont odeint}}

\newcommand{\jQuery}{{\packagefont jQuery}}
\newcommand{\DataTables}{{\packagefont DataTables}}
...
\newcommand{\archivename}{\file{CppTransport\_2016\_03.tar.gz}}
...
\vspace{3mm}
{\small Version \textbf{2017.1}}
\vspace{5mm}\hrule}
\author{\fontseries{s}\selectfont\large David Seery}

\affiliation{\vspace{2mm}\small
...
for inflationary models with canonical kinetic terms
or a nontrivial field-space metric.
...
This guide describes how to install and use the system,
and illustrates its use through some simple examples.
\par\vspace{2cm}
\mbox{}\hfill
\includegraphics[scale=0.1]{LOGO-ERC}
\quad
\includegraphics[scale=0.5]{LOGO-STFC}}
...
\para{Traditional approximation schemes}
...
a `hard' contribution (that is, involving large momenta)
characterized by wavenumbers near the scale
...
equations of motion and is normally the only part to be handled exactly.
The hard contribution is estimated by assuming that
all relevant degrees of freedom are massless and non-interacting.
The archetypal example of such a scheme
is the $\delta N$ formula for the
...
occurs when there is not just one hard scale,
but a number of separated scales.
...
this prescription.
It is the result of four years of development, amounting to roughly 84,000
lines of {\CC},
and consists of three major components:
\begin{enumerate}
    \item
    A \emph{translator} (28,000 lines)
    ...
    and field-space metric (if it is nontrivial).
    ...
    functions.
    The \emph{management system} (34,000 lines) is the largest component
...
\begin{itemize}
    ...
    expansion.%
        ...
        For more details, see the original technical
        paper describing the transport system
        for three-point functions~\cite{DiasFrazerMulryneSeery}.
    ...
    \item Calculations can be performed with \semibold{arbitrary floating-point
    types}, including multiprecision types such as those provided by the
    \href{http://www.mpfr.org}{\packagefont GNU MPFR library}.
    These can be used directly through a {\CC} interface such
    as
    \href{http://www.holoborodko.com/pavel/mpfr/}{\packagefont MPFR \CC}
    or through the
    \href{http://www.boost.org/doc/libs/release/libs/multiprecision}{{\Boost}.{\packagefont Multiprecision}} wrapper.
	This enables calculations to be performed to very high accuracy,
	or with very large numerical range, if that is required.
    ...
    It understands a form of tensor notation
    (including the distinction between co- and contravariant indices)
...
{\CppTransport} supports models in which the scalar field manifold
is given a nontrivial metric $G_{\alpha\beta}$.
In these models, indices on $\delta\phi^\alpha$,
$\pi^\alpha$ and $\delta\pi^\alpha$
are raised and lowered using $G_{\alpha\beta}$ or its inverse
$G^{\alpha\beta}$. The usual summation convention is applied
to contractions such as $\delta\phi^\alpha \delta\phi_\alpha
\equiv \sum_{\alpha,\beta} G_{\alpha\beta} \delta\phi^\alpha
\delta\phi^\beta$.
Note that because $\phi^\alpha$ is simply a scalar function on
the manifold, its index $\alpha$ is \emph{not} a tensor index
and can not be lowered.
...
are $\geq$ gcc 6.2
and $\geq$ Intel 16.0.
Versions of gcc prior to 6.2 have a bug that can prevent linking
with {\Boost}
(although this may not affect you if you compile {\CppTransport}
using the same version used to build {\Boost},
in which case a version $\geq 5.0$ is sufficient),
...
contain bugs that prevent a successful build.%
    ...
    This means that gcc $\geq$ 5.0 should also be available in order
    to provide sufficient {\CC}14 support. Versions of gcc earlier
    than 5.0 do not support {\CC}14 correctly.}
...
The dependencies that must be installed prior to
...
that {\CppTransport} manages internally.
...
\item \semibold{The {\OpenSSL} library.}
This is needed to calculate MD5 hashes, which {\CppTransport} uses
to uniquely identify models. The MD5 algorithm is used to give consistent results
on all platforms.
\end{itemize}
In addition, {\CppTransport} will look for global installations of the following
libraries, although it will fall back to building its own versions if they
are not available:
\begin{itemize}
...
Typically, some of
these dependencies
(such as {\CMake}, an {\MPI} implementation, {\Boost}, {\SQLite}, {\OpenSSL})
will already
...
{\CppTransport} is agnostic about
how these libraries are installed,
but
unless there are compelling reasons to install in some other way
the
packaged versions normally represent
...
\begin{itemize}
    ...
    \begin{center}
        \url{https://zenodo.org/record/61237}
    \end{center}
...
\begin{enumerate}
    ...
    \begin{minted}{bash}
        sudo port install cmake openmpi boost +openmpi ginac openssl
    \end{minted}
...
\para{Using {\Homebrew}}
The procedure is similar for Homebrew.
...
\subsubsection{Installing dependencies on Ubuntu 16.04 and 17.04}
...
Ubuntu 16.04 and 17.04, but the process will be nearly unchanged for any
...
archive by typing
\begin{minted}{bash}
	tar xvf CppTransport_2017_01.tar.gz
\end{minted}
...
with the name \file{CppTransport\_2017\_01}.
(These means you can install the source trees for multiple versions of
{\CppTransport} simultaneously, if required.)
...
\begin{minted}{bash}
	cmake .. -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=~/.cpptransport201701
\end{minted}
This instructs {\CMake}
to build using a release configuration
(some debugging code is suppressed)
and install to the directory \file{\textasciitilde{}/.cpptransport201701}.
...
and
\file{\textasciitilde{}/.cpptransport\_runtime}
...
or similar.
If you install using a unique prefix then it is possible to use
multiple installations of {\CppTransport} simultaneously.
...
\begin{minted}{bash}
	cmake .. -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=~/.cpptransport201701 -DCMAKE_C_COMPILER=icc -DCMAKE_CXX_COMPILER=icpc
\end{minted}
...
\begin{center}
	...
	CppTransport 2017.1 (c) University of Sussex 2016, 2017
\end{center}
...
\begin{itemize}
    ...
    {\CppTransport} allows you to specify default options
    ...
    To set up a simple configuration file,
    create a \file{\textasciitilde{}/.cpptransport} file
    containing a the line such as
    \begin{minted}{text}
        include = /Users/ds283/.cpptransport201701/share/cpptransport
    \end{minted}
    ...
    the symbol \mintinline{text}{~} cannot be used to represent
    the path to the home directory.
    If you wish to search multiple locations then add these as
    extra \mintinline{text}{include} directives on separate lines.

    The runtime system requires a separate configuration
    file called \file{.cpptransport\_runtime}
    which should include the same line:
    \begin{minted}{text}
        include = /Users/ds283/.cpptransport201701/share/cpptransport
    \end{minted}

	In general, the use of configuration files is the recommended approach.
    If you have multiple versions of {\CppTransport} installed then you
    can switch between them by changing the paths used in these files.

    \item \semibold{Use the \envvar{CPPTRANSPORT_PATH} environment variable.}
    Alternatively, you can supply a list of
    locations to search a colon-separated list
    ...
    a line such as
    \begin{minted}{bash}
        export CPPTRANSPORT_PATH=~/.cpptransport201701/share/cpptransport
    \end{minted}
...
(see {\S}\ref{sec:option-summary}), but it is usually more convenient to include it in
the \file{\textasciitilde{}/.cpptransport\_runtime} configuration file.
...
If you are running {\CppTransport} via SSH or a similar remote login, you may need
to force {\Matplotlib} to use a noninteractive backend.
By default {\CppTransport} will use whatever backend has already been configured;
for more details, \href{http://matplotlib.org/faq/usage_faq.html#what-is-a-backend}{see
the {\Matplotlib} documentation}.
If {\Matplotlib} has been configured to use an interactive backend this will usually
fail for jobs started via remote login.
To fix this you should force {\CppTransport} to use a noninteractive backend
using the \option{{-}{-}mpl-backend} option.
The allowed backends are
\mintinline{text}{Agg},
\mintinline{text}{Cairo},
\mintinline{text}{PDF}
and
\mintinline{text}{MacOSX}.
The Cairo renderer sometimes has problems with {\LaTeX}-formatted text. The
\mintinline{text}{Agg} backend is a good choice unless you know you need something
different.
To set this permanently, add line line
\begin{minted}{text}
    mpl-backend = Agg
\end{minted}
to your
\file{\textasciitilde{}/.cpptransport\_runtime} file.
...
\begin{warning}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            an S-matrix element.
            For more details, see~{\S}3.2 of Ref.~\cite{DiasFrazerMulryneSeery}.
...
that declare properties and attributes for the model.
...
`orphaned' code or data that cannot be traced back
...
\begin{itemize}
    ...
    \item \attribute{lagrangian = canonical;} \\
    \attribute{lagrangian = nontrivial_metric;} \\
    Specifies whether the model has a nontrivial field-space metric
    appearing in its kinetic terms. Defaults to
    \attribute{canonical}; if set to \attribute{nontrivial_metric} then
    it is necessary to supply the metric in addition to the potential;
    see below.
    \newfeature{2017.1}
...
\subsection{Adding a metadata block}
You may optionally specify a \block{metadata} block
that controls how {\CppTransport} processes your model file.
Currently there is just a single option:
\begin{itemize}
	\item \attribute{require_version = identifier;} \\
	Specifies the minimum version of {\CppTransport} required to
	process this model file. Where changes in behaviour occur
	between versions,
	{\CppTransport} assumes an `opt-in' model in order not to
	break backwards-compatibility.
	The changes will not be applied unless a
	\attribute{require_version} directive is given.
	So, for example,
	\attribute{require_version = 201701;} will apply any changes
	in behaviour introduced in version 2017.1, but not any subsequent changes.
	Likewise, \attribute{require_version = 201803;} will apply any
	changes introduced up to and included version 2018.3, but not later.
	
	Specifying a minimum version is optional, but recommended. If no
	directive is given then {\CppTransport} will behave like
	version 2016.3 and will issue a warning that
...
Each template is specific to a particular type of Lagrangina.
To use the standard templates for a canonical model the
...
\end{minted}
For a model with nontrivial field-space metric it should be
\begin{minted}{text}
	templates
	  {
...
{\CppTransport} will report an error if you attempt to use
an inappropriate template for your model.
...
class that integrates these equations.
...
that {\CppTransport} needs to solve---
...
The CPU integrator is supplied as templates called
\file{canonical\_mpi.h}
and
\file{nontrivial\_metric\_mpi.h};
in either case, it
writes an output file called
\file{tag\_mpi.h}.
It
...
\begin{itemize}
	...
	This gives the method excellent overall performance.
	...
	\item \option{runge\_kutta\_cash\_karp45}.
	This is an implementation of a $4^{\mathrm{th}}$/$5^{\mathrm{th}}$-order
	Cash--Karp algorithm. Its performance is not competitive, so
	generally \option{runge\_kutta\_dopri5} should be preferred.
	...
	\item \option{adams\_bashforth\_moulton}.
	This is an implementation of the Adams--Moulton multistep method.
	In principle this should be a good choice when the right-hand side
	of the ODE system is expensive to integrate, but this stepper is
	new in {\Boost}.{\odeint} and testing currently shows poor performance.
	For this time being, it should probably be avoided
	until the implementation matures
	unless you know
	what you are doing.
	\newfeature{2017.1}
\end{itemize}
...
\begin{itemize}
    ...
    are reasonable.
    The stepsize will rapidly be adjusted upwards if the solution
...
\end{itemize}
Suitable values for the tolerances are typically in the
range
$10^{-10}$
to
$10^{-14}$,
although in some cases they need to be smaller.
A reasonable default choice is $10^{-12}$, followed by reduction
to $10^{-14}$ or $10^{-16}$ if the stepper fails to keep the solution
under control.
\begin{warning}
	The {\PyTransport} code is an alternative implementation of the
	transport framework, using a mix of Python and {\CC}.
	Although {\CppTransport} and {\PyTransport} apply the same
	formalism, their implementations are \emph{not} identical.
	Internally, they make different choices for the correlation functions
	to be evolved and for the precise numerical scheme used.

	{\CppTransport} and {\PyTransport} have been tested to agree to very
	high precision, so ordinarily these differences are invisible to
	users. However, because they implement different systems of ODEs,
	the meaning of the numerical tolerances
	on each platform \semibold{is not the same}.
	
	In {\CppTransport} the tolerance is the required error in the final
	solution for the field-space correlation functions.
	In {\PyTransport} and internal scaling is applied.
	This has the disadvantage that the meaning
	of the tolerance is not quite so direct,
	but the advantage that larger tolerances can often be used.
	Where {\CppTransport} requires tolerances in the neighbourhood of
	$10^{-12}$, the typical {\PyTransport} equivalent might be in the
	neighbourhood of $10^{-8}$.

	Note that this does not mean that the {\CppTransport} integration
	will be slower.
\end{warning}
...
This always requires an expression for the potential, and
additionally requires the field-space metric in a noncanonical model.
...
\begin{example}
    In double-quadratic inflation there are two
...
we can use them to specify the Lagrangian.

\para{Potential}
It is always necessary to give the potential.
...
\para{Metric}
\newfeature{2017.1}
If a field space metric is required, it should be specified in the form
\begin{minted}{text}
	metric = prefactor * [ component = expression; ... ] + ... ;
\end{minted}
Each component is identified by giving a comma-separated pair of fields,
and the expression is any valid mathematical formula (possibly using
subexpressions as defined above). Any components that are not explicitly
given are assumed to be zero. Off-diagonal elements need be specified only
once; {\CppTransport} uses the symmetry of the metric to determine
any related values.

For example, a flat metric in a model with two fields
\descfile{phi} and \descfile{chi} would be written
\begin{minted}{text}
	model = [ phi, phi = 1; chi, chi = 1; ];	
\end{minted}
whereas a flat metric in polar field-space coordinates
\descfile{R}, \descfile{theta}
would require
\begin{minted}{text}
	model = [ R, R = 1; theta, theta = R^2; ];	
\end{minted}

It is possible to add or subtract multiple metric definitions
to obtain the final formula. For example, this can be useful to help readability
in cases where there multiple sectors in the inflationary field-space
that give a block-diagonal metric.
\begin{minted}{text}
	model = [ phi, phi = 1; chi, chi = 1; ] + 2 * [ xi, xi = 1; xi, mu = 2; mu, mu = xi^2; ] / 3;
\end{minted}
...
\begin{example}
    ...
    \end{minted}
    The model is canonical, so we do not need to specify
...
{\CC} classes that implement the transport equations for it.
...
\begin{itemize}
    ...
    Should be followed by a path to be added to the list
    ...
    Followed by a path that specifies the file to which the
    ...
    to specify a filename explicitly.
\end{itemize}
A further set of options influence the {\CC} code generated by the translator:
\begin{itemize}
    \item \option{{-}{-}no-cse} \\
    ...
    This improves performance, but also gives the compiler maximum opportunity
    to optimize redundant parts of the calculation.
    Taken together, these effects can
    gives better performance at the expense of
    much larger generated code; see the discussion in~\S\ref{sec:codegen}
    below.
\end{itemize}
Finally, there are options to control warnings produced during translation:
\begin{itemize}
	\item \option{{-}{-}profile} \\
	After translation, issue profiling information regarding the time
	required for individual stages of the process, and
	the performance of the expression and lambda caches.
	This is primarily useful for debugging purposes, or to understand
	performance issues if {\CppTransport} takes a long time to process
	your model. (If significant time is reported for common
	subexpression elimination, it may be worth
	disabling it using \option{{-}{-}no-cse} described above; see~\S\ref{sec:codegen}.)
	\newfeature{2017.1}
\end{itemize}
...
as described above.
Options without an argument should be written as
(for example)
\mintinline{text}{no-cse = yes}.
...
\begin{example}
    ...
    \begin{minted}[xleftmargin=0pt,bgcolor=blue!10,linenos=false]{text}
        ...
        CppTransport: translation finished with 1189 replacement rule expansions
        CppTransport: translating '.../templates/canonical_mpi.h' into 'dquad_mpi.h'
        CppTransport: translation finished with 8987 replacement rule expansions
        CppTransport: processed 1 model in time 0.416s
    \end{minted}
    ...
    For the meaning of `replacement rule expansions', see~\S\ref{sec:codegen}.
\end{example}
...
to well-defined `replacement rules'.
...
\subsubsection{Conversion of tensor equations}
\para{Tensor formulae in {\CppTransport}}
In addition to these simple rewriting rules, the translator
must be able to generate code that implements the transport equations
...
\begin{equation}
\begin{split}
    \frac{\mathrm{D} \Sigma^{\alpha\beta}}{\d N}
    & =
    {u^\alpha}_\gamma \Sigma^{\gamma\beta}
    + {u^\beta}_\gamma \Sigma^{\alpha\gamma} \\
    \frac{\mathrm{D} \alpha^{\alpha\beta\gamma}}{\d N}
    & =
...
and $\mathrm{D}/\d N = (\d \phi^\alpha / \d N) \nabla_\alpha$ is the
...
such as \token{U2\_TENSOR[{\textasciicircum}A\_B]} (corresponding to ${u^\alpha}_\beta$)
and \token{U3\_TENSOR[{\textasciicircum}A\_BC]} (corresponding to ${u^\alpha}_{\beta\gamma}$).
The labels \indexset{{\textasciicircum}A\_B} and \indexset{{\textasciicircum}A\_BC}
represent the associated indices,
with `\indexset{\textasciicircum}' labelling contravariant indices and
`\indexset{\_}' labelling covariant indices.
(These variance specifiers are dropped in canonical models where the distinction
is not needed.)
...
as
\begin{minted}{text}
    dSigma[$^A][$^B] $=  $U2_TENSOR[^A_C] * Sigma[$^C][$^B];
    dSigma[$^A][$^B] $+= $U2_TENSOR[^B_C] * Sigma[$^A][$^C];
    
    dSigma[$^a][$^B] $+= - $GAMMA[^a_c] * Sigma[$^c][$^B];
    dSigma[$^A][$^b] $+= - $GAMMA[^b_c] * Sigma[$^A][$^c];
    
    dSigma[MOMENTUM($^a)][$^B] $+= - $GAMMA[^a_c] * Sigma[MOMENTUM($^c)][$^B];
    dSigma[$^A][MOMENTUM($^b)] $+= - $GAMMA[^b_c] * Sigma[$^A][MOMENTUM($^c)];
\end{minted}
The first set of two lines represents the right hand side,
and the upper case indices \indexset{{\textasciicircum}A},
\indexset{{\textasciicircum}B}
and \indexset{{\textasciicircum}C} range over the full phase space of
field and momentum labels.
The remaining sets represent the connexion terms hidden in the
covariant derivative,
and the lower case indices
\indexset{{\textasciicircum}a},
\indexset{{\textasciicircum}b}
and
\indexset{{\textasciicircum}c}
range over field values only. The function
\mintinline{text}{MOMENTUM()} maps a field index to the
corresponding momentum index, and
\token{GAMMA[{\textasciicircum}a\_b]}
represents the combination
$\Gamma^\alpha_{\beta\gamma} \d\phi^\gamma / \d N$.
It has been assumed that the
contravariant two-point function $\Sigma^{\alpha\beta}$ is
...
\indexset{{\textasciicircum}A} and \indexset{{\textasciicircum}B}
label independent components of the
overall matrix equation,
and that the repeated index \indexset{{\textasciicircum}C} is to be summed over.
For models with a nontrivial metric, it enforces that
free indices appear with the same variance assignment on either side
of the symbol
\mintinline{text}{$=} or
\mintinline{text}{$+=} that divides the formula into left- and right-hand
sides.
It also understands how to manipulate indices on the tensor-like objects
\token{U2\_TENSOR[{\textasciicircum}A\_B]}
and
\token{GAMMA[{\textasciicircum}a\_b]}
to obtain the required variance assignment;
had different positioning been required, we could have written
\token{U2\_TENSOR[{\textasciicircum}AB]},
\token{U2\_TENSOR[\_A{\textasciicircum}B]}, and so on.
The transport equation for the three-point function can
be represented similarly.
In general all of this is hidden from the end user,
who can simply use the predefined templates provided with
{\CppTransport}.
...
for this set of equations scales as $\Or(N^3)$.
For the three-point function there are $N^3$ independent equations,
but now each equation contains terms that sum over \emph{two} dummy indices.
Therefore the overall computational cost scales as $\Or(N^5)$.
These costs manifest themselves in two ways.
...
\begin{itemize}
    ...
    and often incur performance penalties.
    Also,
    ...
    This can be very helpful in models where the metric is nontrivial, but many
    (or most) components of the Riemann tensor happen to be zero and do not need to be
    included in summations. For example, this can happen if the metric is flat
    but we are not working in Cartesian coordinates.
    In such cases the compiler can often optimize away
    the extra complexity of the nontrivial metric integrator, giving results that
    are just as fast as a simple canonical code.
    Without unrolling, summations over the Riemann tensor
    will be retained if the compiler cannot deduce that they have no effect.
    ...
    \mintinline{text}{dSigma[$^A][$^B] $= $U2_TENSOR[^A_C] * Sigma[$^C][$^B];}
    into
    \begin{minted}{c++}
        for(int A = 0; A < 2*N; ++A)
          {
...
By default,
{\CppTransport} attempts to compromise between these
strategies.
It will elect to unroll index sets where the result will not
produce too many lines,
with
the upper limit
being set by the value assigned to
...
Even when this has been done there may be further common pieces that
can be extracted.
...
\end{minted}
The local variable \mintinline{c++}{temp_4} represents
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        This is a templated method that requires the name of the
        ...
        smart pointer that manages it is destroyed.
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \cmakevar{dquad} that consists of the
        ...
        enables certain {\CC}14 features that are used by the {\CppTransport}
        platform.
        ...
        If the executable requires instructions that are not available on
...
\mintinline{c++}{write_tasks()} that accepts
two arguments:
...
\begin{advanced}{Callable objects}
    ...
    It is not necessary to use lambdas
...
\begin{itemize}
    ...
    In addition, \file{database.sqlite} stores information about jobs that
...
be computed.
There are two ways to do this.
The simplest, suitable for tasks that do not sample
...
better suited to tasks that sample a large number of configurations.
...
for a task that also samples the 3-point function.
These tables list the data described above, in addition
to some columns that are not displayed in the HTML table.
...
self-consistent.
To display a table that lists the side lengths explicitly
...
\begin{itemize}
    ...
    This is the main data container that stores the output produced by the job.
...
\begin{enumerate}
    ...
    When the accumulated integration products outgrow this capacity
...
will perform recovery for all tasks that are currently registered as in-flight.
...
\begin{itemize}
    ...
    for a bulk \mintinline{sql}{INSERT} of rows from each temporary container
    ...
    \mintinline{sql}{INSERT} performance is roughly independent of the database
    size,
    ...
    \mintinline{sql}{INSERT}s per second.
    ...
    If this happens the worker will stall until the master is free to
    allocate new work.
    ...
    \begin{itemize}
        ...
        \mintinline{c++}{CPPTRANSPORT_STRICT_CONSISTENCY} macro during compilation.
...
\begin{enumerate}
    ...
    \begin{tabular}{p{2.5cm}p{11.2cm}}
        \mintinline{sql}{unique_id} & unique identifier labelling this row \\
    ...
    \begin{tabular}{p{2.5cm}p{11.2cm}}
        \mintinline{sql}{unique_id} & unique identifier labelling this row. Not used
        unless strict data consistency checks are enabled
        by defining the macro
        \mintinline{c++}{CPPTRANSPORT_STRICT_CONSISTENCY} during
...
\begin{enumerate}
    ...
    \mintinline{sql}{threepf_deriv} --- contains
...
(such as \mintinline{sql}{backg}, \mintinline{sql}{twopf_re} and so on)
...
\mintinline{sql}{INSERT} performance during aggregation, by at least a factor of
two but sometimes more.

To enable strict database checks, edit the main header file
\file{transport.h}
to define the macro
\mintinline{c++}{CPPTRANSPORT_STRICT_CONSISTENCY}, or define it by hand
before including any {\CppTransport} headers.
When enabled, this forces {\CppTransport} to use the
\mintinline{sql}{unique_id} fields in each data table.
...
\mintinline{sql}{INSERT} {\SQLite} must check
the existing database to ensure there are no conflicts.
In a worst-case scenario
this means that \mintinline{sql}{INSERT} performance could degrade
...
\mintinline{sql}{PRIMARY KEY} constraints after the database has been constructed;
...
\begin{itemize}
    ...
    at horizon exit.
    The accuracy of these estimates is the principal factor determining accuracy
    ...
    The alternative is to use {\semibold adaptive initial conditions}.
...
that accepts a reference to a \mintinline{c++}{transport::threepf_kconfig}
...
There is also a \mintinline{sql}{MIN} function that can be used to select
...
\end{minted}
To pick a specific time, it is possible to look up the corresponding serial
...
that apply to the corresponding SQL table, as described in~\S\ref{sec:using-integration-container}.
...
So, for example, we could select all wavenumbers $k$ that leave the horizon in
...
that are generated as part of the task, and lists each associated content group:
\begin{minted}[bgcolor=blue!10]{text}
...
the content groups that were used to feed data into its derived products:
\begin{minted}[bgcolor=blue!10]{text}
...
\mintinline{c++}{time_series_plot<>}
and
\mintinline{c++}{wavenumber_series_plot<>}.
...
\subsubsection{Table-specific options}
\label{sec:table-options}
For the table objects
\mintinline{c++}{time_series_table<>}
and
\mintinline{c++}{wavenumber_series_table<>}
there is an option to select the table format:
\begin{itemize}
    \item \mintinline{c++}{vis_toolkit::table_format get_format()} \\
    \mintinline{c++}{set_format(vis_toolkit::table_format f)} \\
    Determine the format of the table. The allowed options are: \\
    \begin{tabular}{p{6.5cm}p{7cm}}
        \mintinline{c++}{vis_toolkit::table_format::justified} & aligned in columns, padded with spaces \\
        \mintinline{c++}{vis_toolkit::table_format::csv} & comma-separated value \\
        \mintinline{c++}{vis_toolkit::table_format::tsv} & tab-separated value
    \end{tabular} \\
    The default format is determined from the filename
    extension. If this is
    \file{.csv} or \file{.tsv}
    then the format is set to CSV or TSV respectively.
    The other allowed formats are
    \file{.txt}, \file{.data}
    and \file{.dat}. These use justified output.
\end{itemize}
...
and \mintinline{c++}{twopf_wavenumber_series} have an additional option that
...
\subsection{Enabling or disabling indices using an \mintinline{c++}{index_selector<>}}
\label{sec:index-selector}

In~\S\S\ref{sec:example-backg-evolve}--\ref{sec:derived-lines}
...
This can be
achieved by increasing the number of e-folds of massless evolution,
...
Such plots are an especially useful
diagnostic tool where they include the subhorizon
...
The normal response should be to tighten the numerical tolerances
(\S\ref{sec:stepper-block}),
increase the number of e-folds of massless evolution
(\S\ref{sec:add-integration-task} and p.\pageref{enum:adpative-ics}), or both.
...
\begin{itemize}
	...
	it in curly braces
	\mintinline{bash}+{+ $\cdots$ \mintinline{bash}+}+.
...
\begin{itemize}
    \item The GinacPrint common subexpression elimination algorithm made available by Doug Baker.
    \begin{center}
        \url{http://www.ginac.de/pipermail/ginac-list/2010-May/001631.html}
    \end{center}
    License: GPL-2 \\
    This is incorporated in the source files
    \begin{center}
    \file{translator/backends/infrastructure/language\_concepts/cse.cpp} \\
    \file{translator/backends/infrastructure/language\_concepts/cse.h}
    \end{center}
\end{itemize}
...
\begin{itemize}
    ...
    License: Mozilla public license (see \file{thirdparty/License/SPLINTER.txt})

    \item {\JsonCpp} \\
    \url{https://github.com/open-source-parsers/jsoncpp} \\
    License: MIT License (see \file{thirdparty/License/JsonCpp.txt})
\end{itemize}
{\CppTransport} also depends on the {\Eigen} library using the version
bundled as part of {\SPLINTER}.
\begin{itemize}
    \item {\Eigen} \\
...
or the installation tarball in the \file{thirdparty/} directory.

The {\CppTransport}
sources do not include source code (or derivatives of the source code)
from this projects. It depends on them only as external resources that are
used by HTML reports.
\begin{itemize}
    \item {\jQuery} \\
    \url{https://jquery.com/download/} \\
    License: MIT License
    (see \file{thirdparty/License/jQuery.txt})

    \item Twitter Bootstrap \\
    \url{http://getbootstrap.com} \\
    License: MIT License
    (see \file{thirdparty/License/Bootstrap.txt})

    \item {\packagefont bootstrap-tab-history} \\
    \url{http://mnarayan01.github.io/bootstrap-tab-history/} \\
    License: Apache License
    (see \file{thirdparty/License/bootstrap-tab-history.txt})

    \item {\DataTables} \\
    \url{https://datatables.net} \\
    License: MIT License
    (see \file{thirdparty/License/DataTables.txt})

    \item {\packagefont Prism.js} \\
    \url{http://prismjs.com} \\
    License: MIT license
    (see \file{thirdparty/License/prism.txt})
\end{itemize}
