...
\subsection{What happens when an integration task is in progress}
...
with configuration, and helps identify regions where it is cheap to add
configurations by comparison with other regions where configurations are
expensive.
...
These intrinsic scalings comprise part of the numerical efficiency of the algorithm.
The other part is determined by how effectively the master process is able to
feed tasks to its workers.
{\CppTransport} tries to adjust its scheduling strategy to respond to environmental
pressures, but this may not be successful.
...
or PDF (\file{.pdf}), or PNG (\file{.png}) if a bitmap format is desired.
...
{\CppTransport} makes default choices which usually prevent significant stalling.
...
\begin{figure}
    ...
    generated by {\CppTransport} \option{{-}{-}gantt} option,
