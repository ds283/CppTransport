...
#define BIND(X) std::bind(&cuda_macros::X, this, std::placeholders::_1)


namespace cuda
  {

    cuda_macros::cuda_macros(translation_unit* u, language_printer& p, std::string l)
      : ::macro_packages::replacement_rule_package(u, p), label(l)
      {
//        printer->set_label(label);
      }

    const std::vector<macro_packages::simple_rule> cuda_macros::get_pre_rules()
      {
    ...
    const std::vector<macro_packages::simple_rule> cuda_macros::get_post_rules()
      {
        std::vector<macro_packages::simple_rule> package;
    ...
    const std::vector<macro_packages::index_rule> cuda_macros::get_index_rules()
      {
    ...
    std::string cuda_macros::args_params(const std::vector<std::string> &args)
      {
    ...
    std::string cuda_macros::args_1index(const std::vector<std::string>& args)
      {
        assert(args.size() == 1);
        ...
        for(int i = 0; i < ncoords; i++)
          {
            out << (i > 0 ? ", " : "") << "double* " << name << "_" << this->fl->flatten(i);
          }
    ...
    std::string cuda_macros::args_2index(const std::vector<std::string>& args)
      {
        assert(args.size() == 1);
        ...
        for(int i = 0; i < ncoords; i++)
          {
            for(int j = 0; j < ncoords; j++)
              {
                out << (i == 0 && j == 0 ? "" : ", ") << "double* " << name << "_" << i << "_" << j;
              }
          }
    ...
    std::string cuda_macros::args_3index(const std::vector<std::string>& args)
      {
        assert(args.size() == 1);
            ...
            for(int j = 0; j < ncoords; j++)
              {
                for(int k = 0; k < ncoords; k++)
                  {
                    out << (i == 0 && j == 0 && k == 0 ? "" : ", ") << "double* " << name << "_" << i << "_" << j << "_" << k;
                  }
              }
          }
