...
unsigned int macro_package::apply_line(std::string& line)
  {
    ...
    assignment_package lvalue_assigner(this->fields, this->parameters, this->order);
    std::vector< std::vector<struct index_assignment> > lvalue_assignments = lvalue_assigner.assign(lhs_indices);

		// lvalue_assignments is a list of possible index assignments for the LHS indices.
		// For each assignment, carry out replacement in 'line_prefix' and then
    // apply summation convention to the rest of the line.

		// lvalue_assignments may be empty if there are no valid assignments (probably one of the index
...
unsigned int macro_package::apply_index(std::string& line, const std::vector<struct index_abstract>& lhs_indices,
    ...
    while(line.find(this->prefix) != std::string::npos)
        ...
        // loop over all possible macro candidates, testing whether we can make a replacement
        for(int i = 0; i < ruleset.size(); i++)
	        {
            ...
            if((pos = line.find(this->prefix + rule.name)) != std::string::npos)
                ...
                // obtain a list of suitable index assignments
		            // note that we may get a null list of assignments if some index ranges are empty.
                assignment_package assign(this->fields, this->parameters, this->order);
                std::vector< std::vector<struct index_assignment> > assgn = assign.assign(indices);

		            if(assgn.size() > 0)  // there are valid assignments
			            {
