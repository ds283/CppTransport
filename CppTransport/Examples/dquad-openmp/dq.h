...
// processed on 19:49:11 on 05 07 2013
...
namespace transport
  {
      ...
      transport::twopf<number> dquad<number>::twopf(const std::vector<double>& ks, double Nstar,
        const std::vector<number>& ics, const std::vector<double>& times)
        {
          ...
          this->write_initial_conditions(real_ics, std::cout);
              ...
              for(int j = 0; j < 2*2; j++)
                {
                  for(int k = 0; k < 2*2; k++)
                    {
                      x[2*2 + (2*2*j)+k] = make_tpf_ic(j, k, real_ks[i], Nstar, ics);
                    }
                }
          ...
          if(__ics.size() == this->N_fields)  // initial conditions for momenta *were not* supplied -- need to compute them
            {
              ...
              __rics.push_back(-2.0*(__Mp*__Mp)/( (M_chi*M_chi)*(chi*chi)+(M_phi*M_phi)*(phi*phi))*(M_phi*M_phi)*phi);
              __rics.push_back(-2.0*(__Mp*__Mp)*(M_chi*M_chi)/( (M_chi*M_chi)*(chi*chi)+(M_phi*M_phi)*(phi*phi))*chi);;
            }
      ...
      double dquad<number>::make_tpf_ic(unsigned int __i, unsigned int __j,
        double __k, double __Nstar, const std::vector<number>& __fields)
        {
          ...
          const auto __Hsq             = -1.0/( 1.0/(__Mp*__Mp)*( (__dphi*__dphi)+(__dchi*__dchi))-6.0)/(__Mp*__Mp)*( (M_chi*M_chi)*(chi*chi)+(M_phi*M_phi)*(phi*phi));
      ...
      void dquad_background_functor<number>::operator()(const std::vector<number>& __x, std::vector<number>& __dxdt, double __t)
        {
          ...
          __dxdt[0]          = __dphi;
          __dxdt[1]          = __dchi;
          __dxdt[2]          =  ( 1.0/(__Mp*__Mp)*( (__dphi*__dphi)+(__dchi*__dchi))-6.0)*__dphi/2.0+( 1.0/(__Mp*__Mp)*( (__dphi*__dphi)+(__dchi*__dchi))-6.0)*(__Mp*__Mp)/( (M_chi*M_chi)*(chi*chi)+(M_phi*M_phi)*(phi*phi))*(M_phi*M_phi)*phi;
          __dxdt[3]          =  __dchi*( 1.0/(__Mp*__Mp)*( (__dphi*__dphi)+(__dchi*__dchi))-6.0)/2.0+( 1.0/(__Mp*__Mp)*( (__dphi*__dphi)+(__dchi*__dchi))-6.0)*(__Mp*__Mp)*(M_chi*M_chi)/( (M_chi*M_chi)*(chi*chi)+(M_phi*M_phi)*(phi*phi))*chi;;
        }
      ...
      void dquad_twopf_functor<number>::operator()(const twopf_state& __x, twopf_state& __dxdt, double __t)
        {
          ...
          __background(2) =  ( 1.0/(__Mp*__Mp)*( (__dphi*__dphi)+(__dchi*__dchi))-6.0)*__dphi/2.0+( 1.0/(__Mp*__Mp)*( (__dphi*__dphi)+(__dchi*__dchi))-6.0)*(__Mp*__Mp)/( (M_chi*M_chi)*(chi*chi)+(M_phi*M_phi)*(phi*phi))*(M_phi*M_phi)*phi;
          __background(3) =  __dchi*( 1.0/(__Mp*__Mp)*( (__dphi*__dphi)+(__dchi*__dchi))-6.0)/2.0+( 1.0/(__Mp*__Mp)*( (__dphi*__dphi)+(__dchi*__dchi))-6.0)*(__Mp*__Mp)*(M_chi*M_chi)/( (M_chi*M_chi)*(chi*chi)+(M_phi*M_phi)*(phi*phi))*chi;;

          // set up components of the u2 tensor
          const auto __u2_0_0 = 0.0;
          const auto __u2_1_0 = 0.0;
          const auto __u2_2_0 =  ( 1.0/(__Mp*__Mp)*( (__dphi*__dphi)+(__dchi*__dchi))-6.0)*(__dphi*__dphi)/(__Mp*__Mp)/2.0+2.0*( 1.0/(__Mp*__Mp)*( (__dphi*__dphi)+(__dchi*__dchi))-6.0)*__dphi/( (M_chi*M_chi)*(chi*chi)+(M_phi*M_phi)*(phi*phi))*(M_phi*M_phi)*phi+( 1.0/(__Mp*__Mp)*( (__dphi*__dphi)+(__dchi*__dchi))-6.0)/(__a*__a)*(__Mp*__Mp)/( (M_chi*M_chi)*(chi*chi)+(M_phi*M_phi)*(phi*phi))*(__k*__k)+( 1.0/(__Mp*__Mp)*( (__dphi*__dphi)+(__dchi*__dchi))-6.0)*(__Mp*__Mp)/( (M_chi*M_chi)*(chi*chi)+(M_phi*M_phi)*(phi*phi))*(M_phi*M_phi);
          const auto __u2_3_0 =  __dchi*( 1.0/(__Mp*__Mp)*( (__dphi*__dphi)+(__dchi*__dchi))-6.0)*__dphi/(__Mp*__Mp)/2.0+( 1.0/(__Mp*__Mp)*( (__dphi*__dphi)+(__dchi*__dchi))-6.0)*( __dphi*(M_chi*M_chi)*chi+__dchi*(M_phi*M_phi)*phi)/( (M_chi*M_chi)*(chi*chi)+(M_phi*M_phi)*(phi*phi));
          const auto __u2_0_1 = 0.0;
          const auto __u2_1_1 = 0.0;
          const auto __u2_2_1 =  __dchi*( 1.0/(__Mp*__Mp)*( (__dphi*__dphi)+(__dchi*__dchi))-6.0)*__dphi/(__Mp*__Mp)/2.0+( 1.0/(__Mp*__Mp)*( (__dphi*__dphi)+(__dchi*__dchi))-6.0)*( __dphi*(M_chi*M_chi)*chi+__dchi*(M_phi*M_phi)*phi)/( (M_chi*M_chi)*(chi*chi)+(M_phi*M_phi)*(phi*phi));
          const auto __u2_3_1 =  ( 1.0/(__Mp*__Mp)*( (__dphi*__dphi)+(__dchi*__dchi))-6.0)*(__Mp*__Mp)*(M_chi*M_chi)/( (M_chi*M_chi)*(chi*chi)+(M_phi*M_phi)*(phi*phi))+(__k*__k)*( 1.0/(__Mp*__Mp)*( (__dphi*__dphi)+(__dchi*__dchi))-6.0)*(__Mp*__Mp)/(__a*__a)/( (M_chi*M_chi)*(chi*chi)+(M_phi*M_phi)*(phi*phi))+2.0*__dchi*( 1.0/(__Mp*__Mp)*( (__dphi*__dphi)+(__dchi*__dchi))-6.0)*(M_chi*M_chi)/( (M_chi*M_chi)*(chi*chi)+(M_phi*M_phi)*(phi*phi))*chi+(__dchi*__dchi)*( 1.0/(__Mp*__Mp)*( (__dphi*__dphi)+(__dchi*__dchi))-6.0)/(__Mp*__Mp)/2.0;
          const auto __u2_0_2 = 1.0;
          const auto __u2_1_2 = 0.0;
          const auto __u2_2_2 =  1.0/(__Mp*__Mp)*( (__dphi*__dphi)+(__dchi*__dchi))/2.0-3.0;
          const auto __u2_3_2 = 0.0;
          const auto __u2_0_3 = 0.0;
          const auto __u2_1_3 = 1.0;
          const auto __u2_2_3 = 0.0;
          const auto __u2_3_3 =  1.0/(__Mp*__Mp)*( (__dphi*__dphi)+(__dchi*__dchi))/2.0-3.0;;
