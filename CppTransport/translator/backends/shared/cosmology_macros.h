...
namespace macro_packages {
  ...
  constexpr unsigned int COSMOLOGY_LAGRANGIANTYPE_TOTALARGS = 0;
  ...
  class replace_tcmb : public replacement_rule_simple
  {
    ...
    class replace_lagrangiantype : public replacement_rule_simple
    {
    public:
        // Constructor
        replace_lagrangiantype(std::string n, translator_data& p, language_printer& prn)
          : replacement_rule_simple(std::move(n), COSMOLOGY_LAGRANGIANTYPE_TOTALARGS),
          data_payload(p),
          printer(prn)
        {
        }
        ...
        // Destructor
        ~replace_lagrangiantype() override = default;
