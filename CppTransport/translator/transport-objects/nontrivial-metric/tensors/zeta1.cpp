    ...
    GiNaC::ex zeta1::compute_component(phase_index i)
      {
        ...
        auto args = this->res.generate_cache_arguments(0, std::array<phase_index, 1>{i}, this->printer);
        args += { idx_i };
    ...
    std::unique_ptr<map_lambda> zeta1::compute_lambda(const index_literal& i)
      {
        ...
        auto args = this->res.generate_cache_arguments(0, std::array<index_literal, 1>{i}, this->printer);
        args += idx_i;
