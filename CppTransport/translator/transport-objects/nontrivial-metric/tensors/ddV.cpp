    ...
    std::unique_ptr<flattened_tensor> ddV::compute(const index_literal_list& indices)
      {
        ...
        return(this->res.ddV_resource(indices[0]->get_variance(), indices[1]->get_variance(), this->printer));
      }
    
    
    unroll_state ddV::get_unroll(const index_literal_list& idx_list)
      {
        const std::array< variance, RESOURCE_INDICES::DDV_INDICES > ij = { idx_list[0]->get_variance(), idx_list[1]->get_variance() };
        
        if(this->res.can_roll_ddV(ij)) return unroll_state::allow;
    ...
    std::unique_ptr<atomic_lambda> ddV::compute_lambda(const index_literal& i, const index_literal& j)
      {
        ...
        auto args =
          this->res.generate_cache_arguments<index_literal>(use_dddV, std::array<index_literal, 2>{i,j}, this->printer);
        args += { idx_i, idx_j };
