...
typedef std::list< GiNaC::symbol > cache_tags;
...
class ginac_cache
  ...
  protected:
		...
		class cache_element
		  ...
		  public:
				...
				// constructor
				cache_element(ExpressionType t, unsigned int i, const cache_tags& a, const GiNaC::ex& e);
		  ...
		  public:
				...
				bool compare(ExpressionType t, unsigned int i, const cache_tags& a, GiNaC::ex& e) const;

		    bool compare(ExpressionType t, unsigned int i, const cache_tags& a, GiNaC::ex& e,
  ...
  public:
		...
		//! is copied to expr
		bool query(ExpressionType t, unsigned int i, const cache_tags& a, GiNaC::ex& e);
    ...
    bool query(ExpressionType t, unsigned int i, const cache_tags& a, GiNaC::ex& e, GiNaC::ex& expected);

		//! store an element in the cache. No collision checking is performed.
		void store(ExpressionType t, unsigned int i, const cache_tags& a, const GiNaC::ex& e);
...
template <typename ExpressionType, unsigned int HashSize>
ginac_cache<ExpressionType, HashSize>::cache_element::cache_element(ExpressionType t, unsigned int i, const cache_tags& a, const GiNaC::ex& e)
	: type(t),
		index(i),
		tags(a),
		expr(e)
...
template <typename ExpressionType, unsigned int HashSize>
bool ginac_cache<ExpressionType, HashSize>::cache_element::compare(ExpressionType t, unsigned int i, const cache_tags& a, GiNaC::ex& e) const
	{
		if(this->type != t) return(false);
		if(this->index != i) return(false);
		if(this->tags.size() != a.size()) return(false);

    cache_tags::const_iterator m;
    cache_tags::const_iterator n;
...
template <typename ExpressionType, unsigned int HashSize>
bool ginac_cache<ExpressionType, HashSize>::cache_element::compare(ExpressionType t, unsigned int i, const cache_tags& a, GiNaC::ex& e,
                                                                   GiNaC::ex& expected, bool& matched) const
	{
    if(this->type != t) return(false);
    if(this->index != i) return(false);
...
template <typename ExpressionType, unsigned int HashSize>
bool ginac_cache<ExpressionType, HashSize>::query(ExpressionType t, unsigned int i, const cache_tags& a, GiNaC::ex& e)
	{
...
template <typename ExpressionType, unsigned int HashSize>
bool ginac_cache<ExpressionType, HashSize>::query(ExpressionType t, unsigned int i, const cache_tags& a, GiNaC::ex& e, GiNaC::ex& expected)
	{
    timing_instrument timer(this->query_timer);
...
template <typename ExpressionType, unsigned int HashSize>
void ginac_cache<ExpressionType, HashSize>::store(ExpressionType t, unsigned int i, const cache_tags& a, const GiNaC::ex& e)
	{
    unsigned int hash = this->hash(static_cast<typename std::underlying_type<ExpressionType>::type>(t), i, a.size());
