...
#include "oscillatory_basic_unrolled.h"
...
const double m      = 15.0 * M_Planck;
const double A      = 0.01;
const double w      = 0.0025 * M_Planck;

const double phi_init = 23.4 * M_Planck;
const double chi_init = 0.499;
...
int main(int argc, char* argv[])
  {
    ...
    std::shared_ptr< transport::oscillatory_basic<double> > model = mgr->create_model< transport::oscillatory_basic<double> >();

    // set up parameter choices
    const std::vector<double>     init_params = { lambda, m, A, w };
    ...
    const double Nmax   = 56.0; // how many e-folds to integrate after horizon crossing
    ...
    transport::initial_conditions<double> ics("quad-1", model, params, init_values, Ninit, Ncross, Npre);
...
//    transport::aggregation_range<double> betas = betas_lo + betas_mid + betas_equi;
    ...
    transport::threepf_fls_task<double> tk3("oscillatory.threepf-1", ics, times, kts, alphas, betas, ThreepfStoragePolicy(), false);
    ...
    transport::zeta_threepf_task<double> ztk3("oscillatory.threepf-1.zeta", tk3);
    ...
    transport::derived_data::time_series_plot<double> tk3_twopf_plot("oscillatory.threepf-1.twopf-time", "twopf-time.pdf");
    ...
    tk3_threepf_lo_group.set_klabel_meaning(transport::derived_data::klabel_type::conventional);

    transport::derived_data::time_series_plot<double> tk3_threepf_lo_plot("oscillatory.threepf-1.threepf-time-lo", "threepf-time-lo.pdf");
    ...
    tk3_threepf_lo_plot.set_legend_position(transport::derived_data::legend_pos::bottom_left);
    ...
    tk3_threepf_hi_group.set_klabel_meaning(transport::derived_data::klabel_type::conventional);

    transport::derived_data::time_series_plot<double> tk3_threepf_hi_plot("oscillatory.threepf-1.threepf-time-hi", "threepf-time-hi.pdf");
    ...
    tk3_threepf_hi_plot.set_legend_position(transport::derived_data::legend_pos::bottom_left);


		// b. TIME EVOLUTION OF BACKGROUND QUANTITIES


    transport::derived_data::background_line<double> tk3_epsilon(tk3, all_times, transport::derived_data::background_quantity::epsilon);
    ...
    transport::derived_data::time_series_plot<double> tk3_SR_objects_plot("oscillatory.threepf-1.SR_objects", "SR_objects.pdf");
    ...
    transport::derived_data::time_series_plot<double> tk3_Hubble_plot("oscillatory.threepf-1.Hubble", "Hubble.pdf");
    ...
    transport::derived_data::time_series_plot<double> tk3_u2_plot("oscillatory.threepf-1.u2", "u2.pdf");
    ...
    transport::derived_data::time_series_plot<double> tk3_u3_plot("oscillatory.threepf-1.u3", "u3.pdf");
    ...
    transport::derived_data::time_series_plot<double> tk3_zeta_twopf("oscillatory.threepf-1.zeta-twopf", "zeta-twopf-time.pdf");
    ...
    tk3_zeta_eq_lo.set_klabel_meaning(transport::derived_data::klabel_type::comoving);
    ...
    tk3_zeta_eq_hi.set_klabel_meaning(transport::derived_data::klabel_type::comoving);
    tk3_zeta_eq_hi.set_use_beta_label(true);

    transport::derived_data::time_series_plot<double> tk3_zeta_sq("oscillatory.threepf-1.zeta-eq", "zeta-eq.pdf");
    ...
    tk3_zeta_sq.set_title_text("3pf of $\\zeta$ near equilateral configurations");
		...
		// set up a table too
    transport::derived_data::time_series_table<double> tk3_zeta_sq_table("oscillatory.threepf-1.zeta-eq.table", "zeta-eq-table.txt");
    ...
    tk3_zeta_redbsp_lo.set_klabel_meaning(transport::derived_data::klabel_type::comoving);
    ...
    tk3_zeta_redbsp_hi.set_klabel_meaning(transport::derived_data::klabel_type::comoving);
    tk3_zeta_redbsp_hi.set_use_beta_label(true);

    transport::derived_data::time_series_plot<double> tk3_redbsp("oscillatory.threepf-1.redbsp-eq", "redbsp-eq.pdf");
...
//    tk3_redbsp.set_abs_y(false);
    ...
    transport::derived_data::time_series_table<double> tk3_redbsp_table = transport::derived_data::time_series_table<double>("oscillatory.threepf-1.redbsp-eq.table", "redbsp-eq-table.txt");
    ...
    transport::derived_data::wavenumber_series_plot<double> tk3_zeta_2spec_plot("oscillatory.threepf-1.zeta-2spec", "zeta-2spec.pdf");
    ...
    transport::derived_data::wavenumber_series_table<double> tk3_zeta_2spec_table("oscillatory.threepf-1.zeta-2spec.table", "zeta-2spec-table.txt");
    ...
    transport::derived_data::wavenumber_series_plot<double> tk3_redbsp_spec_plot("oscillatory.threepf-1.redbsp-spec", "redbsp-spec.pdf");
    ...
    transport::derived_data::wavenumber_series_table<double> tk3_redbsp_spec_table("oscillatory.threepf-1.redbsp-spec-table", "redbsp-spec-table.txt");
		tk3_redbsp_spec_table.add_line(tk3_zeta_redbsp_spec_sq);
    tk3_redbsp_spec_table.add_line(tk3_zeta_redbsp_spec_eq);

    tk3_zeta_redbsp_spec_sq.set_current_x_axis_value(transport::derived_data::axis_value::k_axis);
    tk3_zeta_redbsp_spec_eq.set_current_x_axis_value(transport::derived_data::axis_value::k_axis);

    transport::derived_data::wavenumber_series_plot<double> tk3_redbsp_spec_logplot("oscillatory.threepf-1.redbsp-spec-log", "redbsp-spec-log.pdf");
    ...
    transport::derived_data::wavenumber_series_plot<double> tk3_redbsp_beta_plot("oscillatory.threepf-1.redbsp-beta", "redbsp-beta.pdf");
    ...
    tk3_redbsp_beta_plot.set_abs_y(false);
		tk3_redbsp_beta_plot.set_legend_position(transport::derived_data::legend_pos::centre_left);
    ...
    transport::derived_data::wavenumber_series_plot<double> tk3_redbsp_sqk3_plot("oscillatory.threepf-1.redbsp-sqk3", "redbsp-sqk3.pdf");
    ...
    tk3_zeta_redbsp_sqk3_hi_index.set_current_x_axis_value(transport::derived_data::axis_value::squeezing_fraction_k3_axis);
    ...
    transport::derived_data::wavenumber_series_plot<double> tk3_redbsp_sqk3_index_plot("oscillatory.threepf-1.redbsp-sqk3-index", "redbsp-sqk3-index.pdf");
    ...
    tk3_redbsp_sqk3_index_plot.set_legend_position(transport::derived_data::legend_pos::bottom_left);
    ...
    transport::derived_data::wavenumber_series_plot<double> tk3_redbsp_spec_index_plot("oscillatory.threepf-1.redbsp-spec-index", "redbsp-spec-index.pdf");
    ...
    tk3_zeta_2spec_index.set_klabel_meaning(transport::derived_data::klabel_type::conventional);
    tk3_zeta_2spec_index.set_dimensionless(true);
    tk3_zeta_2spec_index.set_spectral_index(true);

    transport::derived_data::wavenumber_series_plot<double> tk3_zeta_2spec_index_plot("oscillatory.threepf-1.zeta-2spec-index", "zeta-2spec-index.pdf");
    ...
    transport::derived_data::cost_wavenumber<double> tk3_hi_cost(tk3, isosceles_hi_kt);
		tk3_hi_cost.set_current_x_axis_value(transport::derived_data::axis_value::squeezing_fraction_k3_axis);
    ...
    transport::derived_data::wavenumber_series_plot<double> tk3_cost_plot("oscillatory.threepf-1.sqk3-cost", "sqk3-cost.pdf");
    ...
    transport::derived_data::twopf_wavenumber_series<double> tk3_chi_spec(tk3, chi_chi, last_time, all_twopfs);
		tk3_chi_spec.set_klabel_meaning(transport::derived_data::klabel_type::conventional);
    tk3_chi_spec.set_dimensionless(true);

    transport::derived_data::wavenumber_series_plot<double> tk3_chi_spec_plot("oscillatory.threepf-1.chi-spec", "chi-spec.pdf");
    ...
    transport::derived_data::threepf_wavenumber_series<double> tk3_chi_bsp_spec(tk3, chi_chi_chi, last_time, isosceles_squeezed_threepf);
		tk3_chi_bsp_spec.set_klabel_meaning(transport::derived_data::klabel_type::conventional);
		tk3_chi_bsp_spec.set_label_text("$k_3/k_t = 0.99$", "k3/k_t = 0.99");
    tk3_chi_bsp_spec.set_dimensionless(true);
    tk3_chi_bsp_spec.set_current_x_axis_value(transport::derived_data::axis_value::efolds_exit_axis);

    transport::derived_data::wavenumber_series_plot<double> tk3_chi_bsp_spec_plot("oscillatory.threepf-1.chi-bsp-spec", "chi-bsp-spec.pdf");
    ...
    transport::derived_data::threepf_wavenumber_series<double> tk3_chi_bsp_sqk3spec_lo(tk3, chi_chi_chi, last_time, isosceles_lo_kt);
		tk3_chi_bsp_sqk3spec_lo.set_klabel_meaning(transport::derived_data::klabel_type::conventional);
		tk3_chi_bsp_sqk3spec_lo.set_current_x_axis_value(transport::derived_data::axis_value::squeezing_fraction_k3_axis);
    ...
    tk3_chi_bsp_sqk3spec_hi.set_klabel_meaning(transport::derived_data::klabel_type::conventional);
    tk3_chi_bsp_sqk3spec_hi.set_current_x_axis_value(transport::derived_data::axis_value::squeezing_fraction_k3_axis);
    tk3_chi_bsp_sqk3spec_hi.set_label_text("$k_t/k_\\star = \\mathrm{e}^9$", "k_t/k* = exp(9)");
    tk3_chi_bsp_sqk3spec_hi.set_dimensionless(true);

    transport::derived_data::wavenumber_series_plot<double> tk3_chi_bsp_sqk3_plot("oscillatory.threepf-1.chi-bsp-sqk3", "chi-bsp-sqk3.pdf");
    ...
    tk3_chi_spec_index.set_klabel_meaning(transport::derived_data::klabel_type::conventional);
    tk3_chi_spec_index.set_dimensionless(true);
    tk3_chi_spec_index.set_spectral_index(true);

    transport::derived_data::wavenumber_series_plot<double> tk3_chi_spec_index_plot("oscillatory.threepf-1.chi-spec-index", "chi-spec-index.pdf");
    ...
    tk3_chi_bsp_spec_index.set_klabel_meaning(transport::derived_data::klabel_type::conventional);
    ...
    tk3_chi_bsp_spec_index.set_current_x_axis_value(transport::derived_data::axis_value::efolds_exit_axis);

    transport::derived_data::wavenumber_series_plot<double> tk3_chi_bsp_spec_index_plot("oscillatory.threepf-1.chi-bsp-spec-index", "chi-bsp-spec-index.pdf");
    ...
    transport::output_task<double> threepf_output = transport::output_task<double>("oscillatory.threepf-1.output", tk3_twopf_plot);
    ...
    std::cout << "oscillatory.threepf-1 output task:" << '\n' << threepf_output << '\n';
