...
#include "inflection_basic_unrolled.h"
...
int main(int argc, char* argv[])
  {
    ...
    const double Nmax   = 9.0; // how many e-folds to integrate after horizon crossing
    ...
    transport::initial_conditions<double> ics("inflection-1", model, params, init_values, Ninit, Ncross, Npre);
...
//    transport::aggregation_range<double> betas = betas_lo + betas_mid + betas_hi + betas_equi;
    ...
    transport::threepf_fls_task<double> tk3("inflection.threepf-1", ics, times, kts, alphas, betas, ThreepfStoragePolicy(), false);
    ...
    transport::zeta_threepf_task<double> ztk3("inflection.threepf-1.zeta", tk3);
    ...
    std::ostringstream hi_kt_query;
		hi_kt_query << std::setprecision(10) << "ABS(alpha) < 0.001 AND ABS(kt_conventional-" << exp(6.5) << ") < 0.01";
    ...
    std::ostringstream lo_kt_query;
		lo_kt_query << std::setprecision(10) << "ABS(alpha) < 0.001 AND ABS(kt_conventional-" << exp(9.0) << ") < 0.001";
    ...
    transport::derived_data::time_series_plot<double> tk3_twopf_plot("inflection.threepf-1.twopf-time", "twopf-time.pdf");
    ...
    transport::derived_data::time_series_plot<double> tk3_threepf_plot("inflection.threepf-1.threepf-time", "threepf-time.pdf");
    ...
    transport::derived_data::time_series_plot<double> tk3_SR_objects_plot("inflection.threepf-1.SR_objects", "SR_objects.pdf");
    ...
    transport::derived_data::time_series_plot<double> tk3_Hubble_plot("inflection.threepf-1.Hubble", "Hubble.pdf");
    ...
    transport::derived_data::time_series_plot<double> tk3_u2_plot("inflection.threepf-1.u2", "u2.pdf");
    ...
    transport::derived_data::time_series_plot<double> tk3_u3_plot("inflection.threepf-1.u3", "u3.pdf");
    ...
    transport::derived_data::time_series_plot<double> tk3_zeta_twopf("inflection.threepf-1.zeta-twopf", "zeta-twopf-time.pdf");
    ...
    transport::derived_data::time_series_plot<double> tk3_zeta_sq("inflection.threepf-1.zeta-eq", "zeta-eq.pdf");
    tk3_zeta_sq.add_line(tk3_zeta_sq_group);
    tk3_zeta_sq.set_title_text("3pf of $\\zeta$ near equilateral configurations");

		// set up a table too
    transport::derived_data::time_series_table<double> tk3_zeta_sq_table("inflection.threepf-1.zeta-eq.table", "zeta-eq-table.txt");
    ...
    transport::derived_data::time_series_plot<double> tk3_redbsp("inflection.threepf-1.redbsp-eq", "redbsp-eq.pdf");
    ...
    transport::derived_data::time_series_table<double> tk3_redbsp_table = transport::derived_data::time_series_table<double>("inflection.threepf-1.redbsp-eq.table", "redbsp-eq-table.txt");
    ...
    transport::derived_data::wavenumber_series_plot<double> tk3_zeta_2spec_plot("inflection.threepf-1.zeta-2spec", "zeta-2spec.pdf");
    ...
    transport::derived_data::wavenumber_series_table<double> tk3_zeta_2spec_table("inflection.threepf-1.zeta-2spec.table", "zeta-2spec-table.txt");
    ...
    transport::derived_data::wavenumber_series_plot<double> tk3_redbsp_spec_plot("inflection.threepf-1.redbsp-spec", "redbsp-spec.pdf");
    ...
    transport::derived_data::wavenumber_series_table<double> tk3_redbsp_spec_table("inflection.threepf-1.redbsp-spec-table", "redbsp-spec-table.txt");
    ...
    transport::derived_data::wavenumber_series_plot<double> tk3_redbsp_spec_logplot("inflection.threepf-1.redbsp-spec-log", "redbsp-spec-log.pdf");
    ...
    tk3_zeta_redbsp_beta_lo.set_current_x_axis_value(transport::derived_data::axis_value::beta_axis);
		tk3_zeta_redbsp_beta_lo.set_label_text("$k_t/k_\\star = \\mathrm{e}^{6.5}$", "k_t/k* = exp(6.5)");
    ...
    tk3_zeta_redbsp_beta_hi.set_current_x_axis_value(transport::derived_data::axis_value::beta_axis);
		tk3_zeta_redbsp_beta_hi.set_label_text("$k_t/k_\\star = \\mathrm{e}^{9.0}$", "k_t/k* = exp(9.0)");

    transport::derived_data::wavenumber_series_plot<double> tk3_redbsp_beta_plot("inflection.threepf-1.redbsp-beta", "redbsp-beta.pdf");
    ...
    tk3_zeta_redbsp_sqk3_lo.set_current_x_axis_value(transport::derived_data::axis_value::squeezing_fraction_k3_axis);
		tk3_zeta_redbsp_sqk3_lo.set_label_text("$k_t/k_\\star = \\mathrm{e}^{6.5}$", "k_t/k* = exp(6.5)");
    ...
    tk3_zeta_redbsp_sqk3_hi.set_label_text("$k_t/k_\\star = \\mathrm{e}^{9.0}$", "k_t/k* = exp(9.0)");

    transport::derived_data::wavenumber_series_plot<double> tk3_redbsp_sqk3_plot("inflection.threepf-1.redbsp-sqk3", "redbsp-sqk3.pdf");
    ...
    tk3_zeta_redbsp_sqk3_lo_index.set_label_text("$n_{f_{\\mathrm{NL}}} \\;\\; k_t/k_\\star = \\mathrm{e}^{6.5}$", "n_fNL k_t/k* = exp(6.5)");
    ...
    tk3_zeta_redbsp_sqk3_hi_index.set_label_text("$n_{f_{\\mathrm{NL}}} \\;\\; k_t/k_\\star = \\mathrm{e}^{9.0}$", "n_fNL k_t/k* = exp(9.0)");

    transport::derived_data::wavenumber_series_plot<double> tk3_redbsp_sqk3_index_plot("inflection.threepf-1.redbsp-sqk3-index", "redbsp-sqk3-index.pdf");
    ...
    transport::derived_data::wavenumber_series_plot<double> tk3_redbsp_spec_index_plot("inflection.threepf-1.redbsp-spec-index", "redbsp-spec-index.pdf");
    ...
    transport::derived_data::wavenumber_series_plot<double> tk3_zeta_2spec_index_plot("inflection.threepf-1.zeta-2spec-index", "zeta-2spec-index.pdf");
    ...
    transport::derived_data::cost_wavenumber<double> tk3_hi_cost(tk3, isosceles_hi_kt);
		...
		tk3_hi_cost.set_label_text("$k_t = \\mathrm{e}^{9.0}$", "k_t = exp(9.0)");
    ...
    transport::derived_data::cost_wavenumber<double> tk3_lo_cost(tk3, isosceles_lo_kt);
		...
		tk3_lo_cost.set_label_text("$k_t = \\mathrm{e}^{6.5}$", "k_t = exp(6.5)");

    transport::derived_data::wavenumber_series_plot<double> tk3_cost_plot("inflection.threepf-1.sqk3-cost", "sqk3-cost.pdf");
    ...
    transport::derived_data::twopf_wavenumber_series<double> tk3_sigma_spec(tk3, sigma_sigma, last_time, all_twopfs);
		tk3_sigma_spec.set_klabel_meaning(transport::derived_data::klabel_type::conventional);

    transport::derived_data::wavenumber_series_plot<double> tk3_sigma_spec_plot("inflection.threepf-1.sigma-spec", "sigma-spec.pdf");
    ...
    transport::derived_data::threepf_wavenumber_series<double> tk3_sigma_bsp_spec(tk3, sigma_sigma_sigma, last_time, isosceles_squeezed_threepf);
		tk3_sigma_bsp_spec.set_klabel_meaning(transport::derived_data::klabel_type::conventional);
		tk3_sigma_bsp_spec.set_label_text("$k_3/k_t = 0.96", "k3/k_t = 0.96");

    transport::derived_data::wavenumber_series_plot<double> tk3_sigma_bsp_spec_plot("inflection.threepf-1.sigma-bsp-spec", "sigma-bsp-spec.pdf");
    ...
    transport::derived_data::threepf_wavenumber_series<double> tk3_sigma_bsp_sqk3spec_lo(tk3, sigma_sigma_sigma, last_time, isosceles_lo_kt);
		tk3_sigma_bsp_sqk3spec_lo.set_klabel_meaning(transport::derived_data::klabel_type::conventional);
		tk3_sigma_bsp_sqk3spec_lo.set_current_x_axis_value(transport::derived_data::axis_value::squeezing_fraction_k3_axis);
		tk3_sigma_bsp_sqk3spec_lo.set_label_text("$k_3/k_\\star = \\mathrm{e}^{6.5}$", "k_t/k* = exp(6.5)");
    ...
    tk3_sigma_bsp_sqk3spec_hi.set_klabel_meaning(transport::derived_data::klabel_type::conventional);
    tk3_sigma_bsp_sqk3spec_hi.set_current_x_axis_value(transport::derived_data::axis_value::squeezing_fraction_k3_axis);
    tk3_sigma_bsp_sqk3spec_hi.set_label_text("$k_3/k_\\star = \\mathrm{e}^{9.0}$", "k_t/k* = exp(9.0)");

    transport::derived_data::wavenumber_series_plot<double> tk3_sigma_bsp_sqk3_plot("inflection.threepf-1.sigma-bsp-sqk3", "sigma-bsp-sqk3.pdf");
    ...
    tk3_sigma_spec_index.set_klabel_meaning(transport::derived_data::klabel_type::conventional);
    tk3_sigma_spec_index.set_spectral_index(true);

    transport::derived_data::wavenumber_series_plot<double> tk3_sigma_spec_index_plot("inflection.threepf-1.sigma-spec-index", "sigma-spec-index.pdf");
    ...
    tk3_sigma_bsp_spec_index.set_klabel_meaning(transport::derived_data::klabel_type::conventional);
    tk3_sigma_bsp_spec_index.set_label_text("$k_3/k_t = 0.999$", "k3/k_t = 0.999");
    tk3_sigma_bsp_spec_index.set_spectral_index(true);

    transport::derived_data::wavenumber_series_plot<double> tk3_sigma_bsp_spec_index_plot("inflection.threepf-1.sigma-bsp-spec-index", "sigma-bsp-spec-index.pdf");
    ...
    transport::output_task<double> threepf_output = transport::output_task<double>("inflection.threepf-1.output", tk3_twopf_plot);
    ...
    std::cout << "inflection.threepf-1 output task:" << '\n' << threepf_output << '\n';
