    ...
    if(mgr->is_master())
      {
        ...
        std::list<typename transport::repository<double>::output_group_record> output = repo->enumerate_integration_task_output("dquad.threepf-1");
...
//
////    twopf.components_time_history(       &py_plt,   root_path.string() + "/re_k_fields",        twopf_fields);
////    twopf.components_time_history(       &py_plt,   root_path.string() + "/re_k_cross",         twopf_cross);
////    twopf.components_time_history(       &py_plt,   root_path.string() + "/re_k_momenta",       twopf_momenta);
//
