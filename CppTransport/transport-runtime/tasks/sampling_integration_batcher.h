...
// CONSTRUCTOR, DESTRUCTOR
basic_batcher::basic_batcher(const boost::filesystem::path &lp, unsigned int w, unsigned int g, bool no_log)
    : logdir_path(lp),
      worker_group(g),
      worker_number(w)
{
  ...
  if (!no_log)
  {
    ...
    this->log_sink->set_formatter(
        ...
        << " | "
        << boost::log::expressions::attr<basic_batcher::log_severity_level, sampling_batcher_severity_tag>("Severity")
        << " | "
...
protected:
  ...
  // COMPUTATION AGENT
  // compute delegate
  transport::postprocess_delegate<number> compute_agent;
...
public:
  ...
  void push_twopf(unsigned int time_serial, unsigned int k_serial, unsigned int source_serial,
                  const std::vector<number> &twopf_values, const std::vector<number> &backg, transport::twopf_type t);
...
// PUSH_BACKG
template <typename number>
void twopf_sampling_batcher<number>::push_backg(unsigned int time_serial, unsigned int source_serial,
...
// PUSH_TWOPF
template <typename number>
void twopf_sampling_batcher<number>::push_twopf(unsigned int time_serial, unsigned int k_serial, unsigned int source_serial,
                                                const std::vector<number> &twopf_values, const std::vector<number> &backg)
{
...
// PUSH_TENSOR_TWOPF
template <typename number>
void twopf_sampling_batcher<number>::push_tensor_twopf(unsigned int time_serial, unsigned int k_serial,
                                                       unsigned int source_serial, const std::vector<number> &tensor_values)
{
...
// PUSH_BACKG
template <typename number>
void threepf_sampling_batcher<number>::push_backg(unsigned int time_serial, unsigned int source_serial,
                                                  const std::vector<number> &values)
{
  return;
}
...
// PUSH_THREEPF
template <typename number>
void threepf_sampling_batcher<number>::push_threepf(unsigned int time_serial, double t, const transport::threepf_kconfig &kconfig,
