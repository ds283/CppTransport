...
namespace transport
  {
    ...
    master_controller<number>::master_controller(boost::mpi::environment& e, boost::mpi::communicator& w,
      ...
      : environment{e},
        ...
        repo{nullptr},      // repo is populated later, when command line arguments are processed
        i_finder{nullptr},  // same
        p_finder{nullptr},  // same
        journal{static_cast<unsigned int>(w.size()-1)},
        ...
        switch(task.get_task_type())
          {
            case postintegration_task_type::twopf:
              {
                ...
                requirement_type elt{parent_task.get_name(),
                                     make_zeta_twopf_content_specifier(z2pf.get_collect_spectral_data())};
            ...
            case postintegration_task_type::threepf:
              {
                requirement_type elt{parent_task.get_name(), make_zeta_threepf_content_specifier()};
            ...
            case postintegration_task_type::fNL:
              {
                ...
                requirement_type elt{parent_task.get_name(), make_fNL_content_specifier()};
            ...
            switch(t->second.get_type())
              {
                case task_type::integration:
                  {
                    // TODO: think more carefully about which tags should be used here.
                    //  Job descriptors allow tags to be specified on a per-job basis, whereas here we only
                    //  search for the tags that were provided on the command line. At the moment these
                    //  are the same thing, but are there scenarios where they could validly be different?
                    //  If so, which tags should we check for here?

                    // fourth argument false disables raises an exception if no match can be found
                    auto group = (*this->i_finder)(t->first, t->second, this->tags, false);
                    found_match = (group != nullptr);
                    break;
                  }
                ...
                case task_type::postintegration:
                  {
                    ...
                    // fourth argument false disables raises an exception if no match can be found
                    auto group = (*this->p_finder)(t->first, t->second, this->tags, false);
                    found_match = (group != nullptr);
                    break;
                  }
    ...
    void master_controller<number>::validate_tasks()
      {
        ...
        for(auto t = this->job_queue.begin(); t != this->job_queue.end(); /* NOTE! intentionally no update step */)
          {
    ...
    void master_controller<number>::execute_tasks()
      {
        ...
        WorkerPool_Context<number> bundle(
          this->environment, this->world, this->repo.get(), this->journal, this->arg_cache, this->busyidle_timers);
            ...
            case task_type::integration:
              {
                ...
                this->dispatch_integration_task(*int_rec, job);
                break;
              }
            ...
            case task_type::output:
              {
                ...
                this->dispatch_output_task(*out_rec, job);
                break;
              }
            ...
            case task_type::postintegration:
              {
                ...
                this->dispatch_postintegration_task(*pint_rec, job);
                break;
              }
          }
      }
