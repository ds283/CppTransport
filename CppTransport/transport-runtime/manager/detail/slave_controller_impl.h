                ...
                case MPI::NEW_WORK_ASSIGNMENT:
                  {
                    ...
                    // inform batcher that a new assignment is about to get underway
                    batcher.begin_assignment();

                    // set up slave message buffer; this captures any error messages generated by the slave process
                    // during work and arranges for them (a) to be journalled to the log files, and (b)
                    // passed back to the master node for display as part of our final payload
                    ...
                    // context handler arranges for push/pop of message 'context', which here means the current task name
                    slave_message_context msg_ctx(messages, tk->get_name());

                    // keep track of wallclock time used during work
