...
namespace transport
	{
        ...
        class zeta_twopf_wavenumber_series: public wavenumber_series<number>, public zeta_twopf_line<number>
          ...
          public:
            ...
            //! get type
            derived_line_type get_line_type() const override { return derived_line_type::zeta_twopf_wavenumber; }
          ...
          public:
            ...
            //! self-replicate
            zeta_twopf_wavenumber_series<number>* clone() const override { return new zeta_twopf_wavenumber_series<number>(static_cast<const zeta_twopf_wavenumber_series<number>&>(*this)); }
        ...
        data_line_set<number> zeta_twopf_wavenumber_series<number>::derive_lines
          (datapipe<number>& pipe, const tag_list& tags, slave_message_buffer& messages) const
          {
            ...
            auto& z_handle = pipe.new_kconfig_zeta_handle(this->kquery);
            ...
            for(const auto& t_value : t_values)
              {
                zeta_twopf_kconfig_data_tag<number> tag = pipe.new_zeta_twopf_kconfig_data_tag(t_value.serial);
        ...
        void zeta_twopf_wavenumber_series<number>::serialize(Json::Value& writer) const
          {
            ...
            this->wavenumber_series<number>::serialize(writer);
				...
				//! zeta twopf spectral index wavenumber-data line
				template <typename number=default_number_type>
		    class zeta_ns_wavenumber_series: public wavenumber_series<number>, public zeta_ns_line<number>
			    {
		      ...
		      public:
				    ...
				    //! construct a zeta twopf wavenumber data object
				    zeta_ns_wavenumber_series(const zeta_twopf_db_task<number>& tk,
                                      SQL_time_query tq, SQL_twopf_query kq,
                                      unsigned int prec = CPPTRANSPORT_DEFAULT_PLOT_PRECISION);

				    // deserialization constructor
				    zeta_ns_wavenumber_series(Json::Value& reader, task_finder<number>& finder);

				    virtual ~zeta_ns_wavenumber_series() = default;
          ...
          public:
            ...
            //! get type
            derived_line_type get_line_type() const override { return derived_line_type::zeta_ns_wavenumber; }
          ...
          public:
		        ...
		        //! self-replicate
		        zeta_ns_wavenumber_series<number>* clone() const override { return new zeta_ns_wavenumber_series<number>(static_cast<const zeta_ns_wavenumber_series<number>&>(*this)); }
          ...
          protected:
		    ...
		    // derived_line<> is *not* called from time_series<>. We have to call it ourselves.
		    template <typename number>
		    zeta_ns_wavenumber_series<number>::zeta_ns_wavenumber_series
          ...
          : derived_line<number>(make_derivable_task_set_element(tk, precomputed_products(false, true, false, false,
		    ...
		    // derived_line<> is *not* called from time_series<>. We have to call it ourselves.
		    template <typename number>
		    zeta_ns_wavenumber_series<number>::zeta_ns_wavenumber_series(Json::Value& reader, task_finder<number>& finder)
			    : derived_line<number>(reader, finder),
            ...
            // pull time-configuration information from the database
		        ...
		        for(const auto& t_value : t_values)
			        {
				        zeta_ns_kconfig_data_tag<number> tag = pipe.new_zeta_ns_kconfig_data_tag(t_value.serial);
		    ...
		    template <typename number>
		    std::string zeta_ns_wavenumber_series<number>::get_LaTeX_label(double t) const
			    {
		        std::string tag = this->make_LaTeX_tag(t);
		        std::string label;
		    ...
		    template <typename number>
		    std::string zeta_ns_wavenumber_series<number>::get_non_LaTeX_label(double t) const
			    {
		        std::string tag = this->make_non_LaTeX_tag(t);
		        std::string label;
		    ...
		    // derived_line<> is *not* called from time_series<>. We have to call it ourselves.
		    template <typename number>
		    void zeta_ns_wavenumber_series<number>::write(std::ostream& out)
			    {
		    ...
		    // derived_line<> is *not* called from time_series<>. We have to call it ourselves.
		    template <typename number>
		    void zeta_ns_wavenumber_series<number>::serialize(Json::Value& writer) const
			    {
		        writer[CPPTRANSPORT_NODE_PRODUCT_DERIVED_LINE_TYPE] = std::string(CPPTRANSPORT_NODE_PRODUCT_DERIVED_LINE_ZETA_TWOPF_NS_WAVENUMBER_SERIES);
          ...
          public:
            ...
            //! get type
            derived_line_type get_line_type() const override { return derived_line_type::zeta_threepf_wavenumber; }
          ...
          public:
				    ...
				    //! self-replicate
				    zeta_threepf_wavenumber_series<number>* clone() const override { return new zeta_threepf_wavenumber_series<number>(static_cast<const zeta_threepf_wavenumber_series<number>&>(*this)); }
          ...
          protected:
		    ...
		    // derived_line<> is *not* called from time_series<>. We have to call it ourselves.
		    template <typename number>
		    zeta_threepf_wavenumber_series<number>::zeta_threepf_wavenumber_series
		      (const zeta_threepf_task<number>& tk, SQL_time_query tq, SQL_threepf_query kq, unsigned int prec)
          : derived_line<number>(make_derivable_task_set_element(tk, precomputed_products(false, false, true, false,
            ...
            data_line_set<number> lines;
		        ...
		        for(const auto& t_value : t_values)
			        {
				        zeta_threepf_kconfig_data_tag<number> tag = pipe.new_zeta_threepf_kconfig_data_tag(t_value.serial);
          ...
          public:
            ...
            //! get type
            derived_line_type get_line_type() const override { return derived_line_type::zeta_redbsp_wavenumber; }
          ...
          public:
		        ...
		        //! self-replicate
		        zeta_reduced_bispectrum_wavenumber_series<number>* clone() const override { return new zeta_reduced_bispectrum_wavenumber_series<number>(static_cast<const zeta_reduced_bispectrum_wavenumber_series<number>&>(*this)); }
          ...
          protected:
		    ...
		    // derived_line<> is *not* called from time_series<>. We have to call it ourselves.
		    template <typename number>
		    zeta_reduced_bispectrum_wavenumber_series<number>::zeta_reduced_bispectrum_wavenumber_series
		      (const zeta_threepf_task<number>& tk, SQL_time_query tq, SQL_threepf_query kq, unsigned int prec)
          : derived_line<number>(make_derivable_task_set_element(tk, precomputed_products(false, false, false, true,
            ...
            std::string group = this->attach(pipe, tags);
		        ...
		        for(const auto& t_value : t_values)
			        {
				        zeta_reduced_bispectrum_kconfig_data_tag<number> tag = pipe.new_zeta_reduced_bispectrum_kconfig_data_tag(t_value.serial);
