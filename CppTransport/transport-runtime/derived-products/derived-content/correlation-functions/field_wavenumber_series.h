...
namespace transport
    ...
    namespace derived_data
        ...
        //! twopf wavenumber data line
        template <typename number=default_number_type>
        class dlogk_twopf_wavenumber_series: public wavenumber_series<number>, public dlogk_twopf_line<number>
          {
          ...
          public:
            ...
            //! construct a twopf wavenumber-series object
            dlogk_twopf_wavenumber_series(const twopf_db_task <number>& tk, index_selector<2> sel,
                                          SQL_time_query tq, SQL_twopf_query kq,
                                          unsigned int prec = CPPTRANSPORT_DEFAULT_PLOT_PRECISION);

            //! deserialization constructor
            dlogk_twopf_wavenumber_series(Json::Value& reader, task_finder<number>& finder);

            virtual ~dlogk_twopf_wavenumber_series() = default;
          ...
          public:
            ...
            //! get type
            virtual derived_line_type get_line_type() const override { return derived_line_type::dlogk_twopf_wavenumber; }
          ...
          public:
            ...
            //! self-replicate
            virtual dlogk_twopf_wavenumber_series<number>* clone() const override { return new dlogk_twopf_wavenumber_series<number>(static_cast<const dlogk_twopf_wavenumber_series<number>&>(*this)); }
        ...
        // derived_line<> is *not* called from time_series<>. We have to call it ourselves.
        template <typename number>
        dlogk_twopf_wavenumber_series<number>::dlogk_twopf_wavenumber_series
          (const twopf_db_task <number>& tk, index_selector<2> sel, SQL_time_query tq, SQL_twopf_query kq,
           unsigned int prec)
        ...
        // derived_line<> is *not* called from time_series<>. We have to call it ourselves.
        template <typename number>
        dlogk_twopf_wavenumber_series<number>::dlogk_twopf_wavenumber_series(Json::Value& reader, task_finder<number>& finder)
          : derived_line<number>(reader, finder),
        ...
        template <typename number>
        data_line_set<number> dlogk_twopf_wavenumber_series<number>::derive_lines
          (datapipe<number>& pipe, const tag_list& tags, slave_message_buffer& messages) const
          {
            unsigned int N_fields = this->gadget.get_N_fields();
                        ...
                        if(this->active_indices.is_on(index_set))
                          {
                            cf_kconfig_data_tag<number> tag =
                              pipe.new_cf_kconfig_data_tag(cf_data_type::cf_twopf_dlogk_re,
                                                           this->gadget.get_model()->flatten(m, n), t_value.serial);
        ...
        template <typename number>
        std::string dlogk_twopf_wavenumber_series<number>::get_LaTeX_label(unsigned int m, unsigned int n, double t) const
          {
            std::string tag = this->make_LaTeX_tag(t);
            std::string label;
        ...
        template <typename number>
        std::string dlogk_twopf_wavenumber_series<number>::get_non_LaTeX_label(unsigned int m, unsigned int n, double t) const
          {
            std::string tag = this->make_non_LaTeX_tag(t);
            std::string label;
        ...
        // derived_line<> is *not* called from time_series<>. We have to call it ourselves.
        template <typename number>
        void dlogk_twopf_wavenumber_series<number>::write(std::ostream& out)
          {
        ...
        // derived_line<> is *not* called from time_series<>. We have to call it ourselves.
        template <typename number>
        void dlogk_twopf_wavenumber_series<number>::serialize(Json::Value& writer) const
          {
            writer[CPPTRANSPORT_NODE_PRODUCT_DERIVED_LINE_TYPE] = std::string(CPPTRANSPORT_NODE_PRODUCT_DERIVED_LINE_DLOGK_TWOPF_WAVENUMBER_SERIES);
